...
public:
    // Accessors for DMR slot address metadata (src ID / dst ID / group flag).
    // DMRAddresses is defined in the private section above; these inline
    // methods are valid here because the struct is fully declared by this point.
    const DMRAddresses& getSlot1Addresses() const { return m_slot1Addresses; }
    const DMRAddresses& getSlot2Addresses() const { return m_slot2Addresses; }
};
