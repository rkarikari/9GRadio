...
public:
    ...
    /** Diagnostic status (error counts, TONE/ERASURE/REPEAT/MUTE flags) from the last slot-1 vocoder frame. */
    const mbe_process_result& getLastMbeResult1() const { return m_mbeDecoder1.getLastResult(); }
    /** Diagnostic status (error counts, TONE/ERASURE/REPEAT/MUTE flags) from the last slot-2 vocoder frame. */
    const mbe_process_result& getLastMbeResult2() const { return m_mbeDecoder2.getLastResult(); }
    ...
    const DSDP25P1& getP25P1Decoder() const { return m_dsdP25P1; }
