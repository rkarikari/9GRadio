...
public:
    ...
    /**
     * Diagnostic status from the most recently processed frame: corrected
     * error counts and MBE_PROCESS_FLAG_* bits (TONE/ERASURE/REPEAT/MUTE/
     * C0_VALID/...). Populated on every processFrame()/processData() call
     * so callers (e.g. the JNI debug telemetry layer) can see exactly what
     * the vocoder is doing frame-by-frame without guessing from audio.
     */
    const mbe_process_result& getLastResult() const { return m_lastResult; }
...
private:
    ...
    mbe_process_result m_lastResult; //!< diagnostic status from the last processFrame()/processData() call
