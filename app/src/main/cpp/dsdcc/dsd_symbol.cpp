...
void DSDSymbol::snapLevels(int nbSymbols)
{
    ...
    //
    // Matches the classic DSD / dsd-neo decision-threshold constant
    // (state->umid/lmid in engine.c): the outer/inner cluster boundary is
    // placed at 5/8 of the half-range from center to the measured peak, not
    // at the theoretical ideal-C4FM midpoint (2/3) and not at the
    // under-margined 1/2. dsd-neo's 5/8 constant is empirically tuned across
    // real P25/DMR/YSF signals (accounting for pulse shaping, ISI and typical
    // FM deviation error), and every decoder in this port that shares
    // dsd-neo's FEC tables needs matching symbol-slicing behavior for the
    // downstream frame sync and FEC to see the bit error rates they were
    // designed around. Deviating from 5/8 (this file previously used 2/3)
    // shifts the inner-symbol decision boundary enough to systematically
    // misclassify dibits under normal channel noise -- frame sync still
    // locks (sync patterns tolerate some bit errors) but IMBE/AMBE voice
    // payload dibits get flipped, producing garbled (not silent) audio.
    m_center = (m_max + m_min) / 2;
    m_umid   = m_center + (m_max - m_center) * 5 / 8;
    m_lmid   = m_center - (m_center - m_min) * 5 / 8;
}
...
void DSDSymbol::snapMinMax()
{
    ...
    // Recalibrate center/umid/lmid. See snapLevels() above for why 5/8
    // (matching dsd-neo's state->umid/lmid) is correct and 2/3 is not.
    m_center = (m_max + m_min) / 2;
    m_umid   = m_center + (m_max - m_center) * 5 / 8;
    m_lmid   = m_center - (m_center - m_min) * 5 / 8;
}
